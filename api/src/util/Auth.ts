...
export class Auth {
    ...
    static async validacaoUsuario(req: Request, res: Response): Promise<any> {      
        ...
        try {
            ...
            const queryResult = await database.query(querySelectUser, [username, senha]); //username >> email
            ...
            if (queryResult.rowCount != 0) {
                ...
                const usuario = {
                    ...
                    username: queryResult.rows[0].username //email
                }

									// Gera o token do usuário, passando como parâmetro as informações do objeto professor
                const tokenUsuario = Auth.generateToken(parseInt(usuario.id_usuario), usuario.nome, usuario.username); //email
     ...
     */
    static generateToken(id: number, nome: string, username: string) { //username >> email
        ...
        return jwt.sign({ id, nome, username }, SECRET, { expiresIn: '1h' }); //username >> email
    }
